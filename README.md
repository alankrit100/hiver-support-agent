...
2. **The ensemble's gain over the classifier alone (57.8% → 68.9%) depends on a live API responding well at eval time.** We personally hit this during development: a Sarvam credit outage caused 36/45 few-shot predictions to silently degrade to `unclear`/0.0 confidence, producing a corrupted, misleadingly *higher-looking* run before we caught it and re-ran cleanly. The reported number is from the clean run, but the fragility itself is real and worth disclosing — this isn't a fixed, guaranteed gain, it's contingent on the LLM backend actually working that day.
